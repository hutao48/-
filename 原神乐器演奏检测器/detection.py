...
class NoteDetector:
    ...
    def _process_frame(self, frame: np.ndarray, t_now: float) -> dict:
        ...
        midis = self._peaks_to_notes(peaks, mag)

        # 谐波结构校验：真实乐器音必须带谐波，噪声毛刺无谐波 → 丢弃
        midis = [m for m in midis if self._has_harmonic_support(m, mag)]
        ...
        # 谐波去重：整数倍频只保留基频
        midis = self._dedup_harmonics(midis)
    ...
    def _has_harmonic_support(self, midi, mag):
        ...
        for k in (2, 3, 4):
            ...
            if float(mag[lo:hi].max()) > 0.12 * base_amp:
                return True
        return False

    def _dedup_harmonics(self, midis):
